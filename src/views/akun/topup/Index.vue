    ...
    <form action="#" @submit.prevent="deposit()" method="post" class="mt-4">
      <RadioGroup v-model="form.nominal" class="mt-2 mb-8">
        ...
        <div class="grid grid-cols-2 gap-3 sm:grid-cols-6">
          <RadioGroupOption
            as="template"
            v-for="option in nominalOptions"
            :key="option.name"
            :value="option.nominal"
          ...
          >
            <div
              :class="[
                ...
                checked
                  ? 'bg-sky-400 border-transparent text-white hover:bg-sky-700'
                  : 'bg-white border-gray-200 text-gray-900 hover:bg-gray-50',
                'border rounded-md py-3 px-3 flex items-center justify-center text-xs font-bold uppercase sm:flex-1',
              ]"
            >
      ...
      <div class="flex justify-end">
        <button
          type="submit"
          class="w-30 flex items-center justify-center bg-sky-400 p-3 text-white text-xs rounded-md"
        >
  ...
  <div class="mx-auto bg-white rounded-md mt-4">
    <ul role="list" class="divide-y divide-gray-200">
      <li v-for="item in deposits.data" :key="item.id">
        <a
          @click="depositDetail(item.snap_token)"
          ...
          <div class="px-2 py-2">
            ...
            <div class="flex items-center justify-between">
              ...
              <div class="ml-2 flex flex-shrink-0">
                ...
                <p
                  v-if="item.payment_status == '3'"
                  class="inline-flex rounded-full bg-red-100 px-2 text-xs font-semibold leading-5 text-red-800"
                >
                  Expired
                </p>
                <p
                  v-if="item.payment_status == '4'"
                  class="inline-flex rounded-full bg-red-100 px-2 text-xs font-semibold leading-5 text-red-800"
                >
                  Batal
                </p>
